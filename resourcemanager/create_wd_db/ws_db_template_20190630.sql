...
--
-- Host: localhost    Database: cloud_docs
-- ------------------------------------------------------
...
--
-- Dumping events for database 'cloud_docs'
--

--
-- Dumping routines for database 'cloud_docs'
--
...
DELIMITER ;;
CREATE  FUNCTION `fn_CheckUserDocPermission`(
	user_id int,
    document_id int,
...
BEGIN
	DECLARE has_permissions_itself INTEGER DEFAULT (
		...
		-- INNER JOIN security_group sg on ds.security_group_id = sg.id
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  FUNCTION `fn_GetUserDocPermissionsMask`(
	user_id int,
    document_id int
    ) RETURNS varchar(255) CHARSET utf8
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_AddDocToParentDoc`(
	user_id int,
	document_id int,
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `prc_clearDocPermissions` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_clearDocPermissions`(
	grantor_user_id int,
	document_id int
)
BEGIN
	DECLARE user_id int;
	DECLARE user_secgroup int;
    
    DECLARE author_user_id INTEGER DEFAULT (SELECT author_user_id from document WHERE id = document_id);

	IF fn_CheckUserDocPermission(grantor_user_id, document_id, 'S') = 'none' THEN
		SELECT -1 as 'result';
	ELSE
		DELETE FROM document_security
		WHERE document_security.document_id = document_id 
			AND document_security.security_group_id NOT IN 
            (SELECT sgid FROM (
             SELECT ds.security_group_id as 'sgid' from document_security ds
             INNER JOIN security_group sg on ds.security_group_id = sg.id
             INNER JOIN user_security_group_cache usg on usg.security_group_id = sg.id
			 WHERE usg.user_id = grantor_user_id
            ) as ctable)
            AND document_security.security_group_id IN
            (
				SELECT id from security_group WHERE `primary` = 1
            );
	END IF;
END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_ClearDocSecurityGroupPermissions`(
	grantor_user_id int,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_ClearImplicitReadPermissionFromParentDocs`(
    security_group_id int,
    document_id int
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_ClearUserSecurityGroupsCache`(
user_id int
)
BEGIN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `prc_DeleteDoc` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_DeleteDoc`(
	user_id int,
	delete_doc_id int
)
BEGIN
	...
	ELSEIF fn_CheckUserDocPermission(user_id, delete_doc_id, 'D') = 'none' THEN
		SELECT -2 as 'deleteCount'; -- insufficient permissions
	ELSE
		IF parent_doc_id IS NOT NULL THEN
			CALL prc_RemoveDocFromParentDoc(user_id, delete_doc_id, parent_doc_id);
        END IF;
		
		START TRANSACTION;

		-- 0. Delete the 'I' permissions generated by all descending docs
		DELETE ds1 FROM document_security ds1 JOIN
		document_security ds2 ON ds1.inherited_by_relationship_id = ds2.relationship_id
		WHERE ds2.document_id IN (SELECT dt.document_id FROM document_tree dt WHERE path LIKE CONCAT('%/', delete_doc_id, '%'));
        
		-- 1. Delete the permissions of all descending docs
         DELETE FROM document_security WHERE document_security.document_id IN 
		 (SELECT dt.document_id FROM document_tree dt WHERE path LIKE CONCAT('%/', delete_doc_id, '%'));

		-- 2. Delete all descending docs
		DELETE FROM document WHERE id IN (SELECT dt.document_id FROM document_tree dt WHERE path LIKE CONCAT('%/', delete_doc_id, '%'));

		-- 3. Delete all descending docs from the docment_tree
		DELETE FROM document_tree WHERE path LIKE CONCAT('%/', delete_doc_id, '%');

		-- 4. Delte the permissions for the document
        DELETE FROM document_security WHERE document_security.document_id = delete_doc_id;
        
        -- 5. Delte the document from the document_tree
        DELETE FROM document_tree WHERE document_tree.document_id = delete_doc_id;

		-- 6. Delete the document itself
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_DeleteDocSelf`(
	user_id int,
	delete_doc_id int
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_GetChildDocs`(
	user_id int,
	parent_document_id int
)
BEGIN
...
BEGIN
    ...
    IF is_anon_doc > 0 THEN
		...
		-- INNER JOIN security_group sg on ds.security_group_id = sg.id
		INNER JOIN user_security_group_cache usg on usg.security_group_id = ds.security_group_id
		WHERE d.id = docId AND
			(usg.user_id = user_id OR usg.user_id = anon_user_id) AND
			(ds.permissions_mask LIKE '%R%')
		-- get the one with the least restrictive permissions
		ORDER BY LENGTH(ds.permissions_mask) DESC
		LIMIT 0, 1;
    ELSE 
		SELECT 
			d.*,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_GetDocPath`(
	document_id int
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_GetDocSecurityInfo`(
	document_id int
)
BEGIN
	SELECT 
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_GetDocsList`(
	user_id int,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_isDocSharedEveryone`(
	document_id int
)
BEGIN
	DECLARE anon_user_id INT DEFAULT -1;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `prc_PublicDocsList` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_PublicDocsList`()
BEGIN
	SELECT
		d.*,
		ds.permissions_mask
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_RemoveDocFromParentDoc`(
	user_id int,
	document_id int,
	parent_document_id int
)
BEGIN
	...
	IF fn_CheckUserDocPermission(user_id, document_id, 'S') = 'none' AND
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_RemoveSecurityGroupsPermissionsPropagatedToParentDocs`(
	document_id int
)
BEGIN
	DELETE ds1 FROM document_security ds1 JOIN
...
BEGIN
    ...
    ELSE
		...
		ELSE  -- existing document	
			-- check if the user has permissions to modify
			IF fn_CheckUserDocPermission(user_id, document_id, 'W') <> 'none' THEN
				UPDATE document SET
					`type` = doctype,
					`name` = docname,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_SetDocSecurityGroupPermissions`(
	grantor_user_id int,
	security_group_id int,
	document_id int,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_SetImplicitReadPermissionToParentDocs`(
	grantor_user_id int,
    security_group_id int,
    document_id int,
...
BEGIN
	...
	label1: LOOP
		...
		IF LENGTH(token) > 0 THEN
			...
			-- SET out_path = CONCAT(out_path, '|', token);
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_SetUserSecurityGroupsCache`(
user_id int,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_shareDocEveryone`(
	user_id int,
	document_id int
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `prc_unshareDocEveryone`(
	user_id int,
	document_id int
)
BEGIN
...
-- Dump completed on 2019-06-30 10:30:53
